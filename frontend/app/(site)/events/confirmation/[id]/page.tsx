...
export default async function EventConfirmationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [booking, settings] = await Promise.all([eventApi.getBooking(id).catch(() => null), settingsApi.getSettings()]);
  if (!booking) notFound();
    ...
    <main className="mx-auto max-w-xl px-4 py-20 text-center sm:px-6">
      ...
      <p className="mt-2 text-stone-500">
        {paid
          ...
          : offlinePending
            ? "We've held your tickets. Pay to the below details, then send your receipt to confirm."
